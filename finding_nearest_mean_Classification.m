...
condition2_start = size(HCTSA_18s,1)/2;  % dependent on the number of conditions availble in the data set
condition_one= HCTSA_18s(1:condition2_start,:);

%
%% (1) finding the nearest mean
    ...
    idx = 1;
       for channel_idx = 1:num_channel
        result_idx = 1;
            ...
            ele_condition1_under_inv = condition1_under_inv(fly_idx); % cross-validation leave-one-out
            condition1_under_inv(fly_idx) = [];
          con1_flies_lo_ch(:,fly_idx,channel_idx,op_idx) = condition1_under_inv;
          
            mean_condition1_under_inv = nanmean(condition1_under_inv);    % method
           % for fly_idx_comb = 1:num_fly
            ...
            con2_flies_lo_ch(:,fly_idx,channel_idx,op_idx) = condition2_under_inv;
                ...
                mean_condition2_under_inv = nanmean(condition2_under_inv);   %method
...
%% getting threshold
  for ch= 1:num_channel
    ...
    condition_1= con1_flies_lo_ch(:,flies,ch,iter); 
      ...
      means_condition1_under_inv(:,flies,ch,iter) = nanmean(condition_1);  % change method

    condition_2= con2_flies_lo_ch(:,flies,ch,iter);
    
  means_condition2_under_inv(:,flies,ch,iter) = nanmean(condition_2);     %change method
...
%get threshold mat an assign it a (z)
%get mat that contain condition 1 files and assign it as condition_1 sorted
%as 13 fles equal one channel sequntially
